...
Original evaluated system: commit `6772707`. Hardened system: commit `81bafe4`.
...
| committed vs hardened | 800 | 0 | 0 | 0 | 0 | 0 | 800 |
| original vs hardened | 800 | 0 | 0 | 0 | 0 | 0 | 800 |

Compared fields: escalate, reason, intent, reply, security_sensitive, context_sufficient, evidence, grounding_passed, intent_confidence, retrieval_confidence, model_confidence. Score fields did not exist at the evaluation commit, so the original comparisons cover the remaining fields (not comparable: intent_confidence, retrieval_confidence, model_confidence). Latency and token usage are excluded: they are not outputs.

Negative control (one flipped decision is reported): PASS.
